...
import type { Crawl4aiNodeOptions, Crawl4aiApiCredentials, CssSelectorSchema } from '../helpers/interfaces';
import {
				...
				items.forEach(item => {
					Object.entries(item).forEach(([key, value]) => {
						if (typeof value === 'string') {
							// Clean text: remove extra spaces, normalize whitespace
							item[key] = value
								.replace(/\s+/g, ' ')
								.trim();
						}
