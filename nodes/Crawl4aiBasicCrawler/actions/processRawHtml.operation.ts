...
import { createCrawlerInstance, createCrawlerRunConfig, formatCrawlResult } from '../helpers/utils';
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		try {
			...
			// Create URL with raw:// prefix to indicate HTML content
			const rawUrl = `raw://${html}`;

			// Run the processing
			const result = await crawler.arun(rawUrl, {
				...crawlerConfig,
				baseUrl,
			});
