...
- `DynamicStructuredTool` now tries the n8n-owned-tree anchor *before* the filesystem anchor (not after) — if npm auto-installs this package's declared `@langchain/core` peerDependency as a private local copy, the filesystem anchor would otherwise resolve and memoize that copy first, a different class identity than n8n's own. The filesystem anchor also now rejects any match nested inside this package's own `node_modules`.
